...
function BaseWriter() {
  this.engine = getEngine(option.get('engine'));
}

BaseWriter.prototype.start = function() {
  log.info('start', this.constructor.name);
  this.setup && this.setup();
  return this;
};

BaseWriter.prototype.stop = function() {
  log.info('stop', this.constructor.name);
  this.run && this.run();
  return this;
};
...
BaseWriter.prototype.render = function(data) {
  ...
  var html = this.engine.render(data.template, data.params);
...
BaseWriter.prototype.writeIframes = function(destination, iframes) {
  ...
  var writeIframe = function(item) {
    ...
    // some functions need to know the params
    data.params = self.registerContextFunctions(data.params);

    var html = self.engine.render('iframe.html', data.params);
    file.write(destination, html);
  };
...
BaseWriter.prototype.registerContextFunctions = function(ctx) {
  ...
  var contextfunctions = option.get('contextfunctions');
