...
file.recurse = function recurse(rootdir, callback, subdir, filter) {
  ...
  fs.readdirSync(abspath).forEach(function(filename) {
    ...
    if (option.get('ignorecvs') && /^\.(git|hg|svn)$/.test(subdir)) {
      return;
    }
    // ignore cachedir directory
    if (file.contain(file.abspath(option.get('cachedir')), filepath)) {
      return;
    }
...
file.read = function(filepath) {
  ...
  return fs.readFileSync(filepath, option.get('encoding'));
};

file.readJSON = function(filepath) {
  // use comptable rather than parse json
  log.debug('json', file.cleanpath(filepath));
  try {
