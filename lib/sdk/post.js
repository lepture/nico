...
exports.index = function(post) {
  ...
  data[filename] = {title: post.title, meta: post.meta};
  file.write(cacheindex, JSON.stringify(data));
  exports.emit('indexed', post);
  return data;
};
...
exports.load = function(sourcedir) {
  ...
  exports.emit('loading', sourcedir);
  ...
  file.recurse(sourcedir, function(filepath) {
    ...
    if (!file.exists(runtimefile) ||
        file.stat(filepath).mtime > file.stat(runtimefile).mtime) {
  ...
  // record last runtime
  var data = file.readJSON(runtimefile) || {};
  data.loaded = new Date();
  file.write(runtimefile, JSON.stringify(data));

  exports.emit('loaded', sourcedir);
  return true;
};
