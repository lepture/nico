...
exports.YearWriter = BaseWriter.extend({
  ...
  run: function() {
    ...
    _.each(years, function(value, key) {
      var posts = _.sortBy(value, function(item) { return item.pubdate; });

      self.render({
        destination: key + '/index.html',
        params: {posts: posts.reverse()},
        template: self.storage.config.year_template || 'archive.html'
      });
    });
  }
});
