...
function initSwig(obj) {
  ...
  var swigConfig = obj.swigConfig || obj.config.swigConfig;
  swigConfig = swigConfig || {};
  ...
  for (var key in swigConfig.filters) {
    var func = swigConfig.filters[key];
    if (_.isString(func)) {
...
var BaseWriter = Class.create({
  ...
  render: function(obj) {
    ...
    this.registerFunctions(obj.params);
  ...
  registerFunctions: function(ctx) {
    var key, func;
    var contextfunctions = this.storage.config.contextfunctions;
    ...
    var functions = this.storage.config.functions;
  ...
  registerContextFilters: function(ctx) {
    var filters = this.storage.config.contextfilters;
    if (!filters) return;
