...
function Post(obj) {
  ...
  this.relative_filepath = utils.relativePath(this.filepath, obj.root);
  this.parser = obj.parser || new MarkdownParser();

  var content = obj.content;
  if (!content) {
    logging.info('reading content from %s', this.relative_filepath);
...
function getToc(content) {
  ...
  renderer.header = function(text, level) {
    var id = utils.encodeURIComponent(text);
...
function getHtml(content, id) {
  ...
  renderer.blockcode = function(code, language) {
    ...
    html += util.format(
      '<pre><code class="%s">%s</code></pre>', language, code
    );
    return html;
  };
