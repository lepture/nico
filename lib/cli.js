...
exports.build = function(prog) {
  ...
  storage.swigConfig.globals = storage.swigConfig.globals || {};
  storage.swigConfig.globals.system = require('../package.json');
  var theme;
  try {
    theme = require(path.join(storage.config.theme, 'theme.js'));
  } catch (e) {
    logging.warn('There is no theme config file');
  }
  if (theme) storage.swigConfig.globals.theme = theme;
